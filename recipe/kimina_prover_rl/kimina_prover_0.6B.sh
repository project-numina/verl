...
python3 -m verl.trainer.main_ppo \
    ...
    data.train_files="[$WORKING_DIRECTORY/prompt_sets/$PROMPT_SET_TRAIN/train.parquet]" \
    data.val_files="[$WORKING_DIRECTORY/prompt_sets/$PROMPT_SET_TRAIN/test.parquet]" \
    data.train_batch_size=256 \
    data.max_prompt_length="${max_prompt_length}" \
    data.max_response_length="${max_response_length}" \
    +data.return_extra_info=True \
    +data.multiturn=True \
    +data.multiturn_sampling_rate=0.5 \
    +data.multiturn_n_samples_in_cache=5000 \
    ...
    actor_rollout_ref.model.path=AI-MO/Kimina-Prover-Distill-0.6B \
    ...
    trainer.experiment_name='kimina-prover-0.6B' \
