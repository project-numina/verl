...
class ChatCompletionScheduler:
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        # ─── 2.  launch every prompt (possibly repeated) ──────────────────
        tasks, batch_conversations = [], [None] * len(batch) * n
        all_sequences_turn_data   = [{}  for _ in range(len(batch) * n)]

        for i, conversation in enumerate(batch.non_tensor_batch["raw_prompt"].repeat(n, axis=0)):
            batch_conversations[i] = conversation.tolist()
            tasks.append(
                asyncio.create_task(
                    self._submit_chat_completions_semaphore(
                        messages=batch_conversations[i],
                        request_id=None,
                        sampling_params=kwargs,
                        turn_data=all_sequences_turn_data[i],
                        extra_info_for_item=batch.non_tensor_batch["extra_info"][i // n],
                    )
                )
            )
        ...
        sizes = [len(td) if td else 0 for td in all_sequences_turn_data]
        print(f"[DEBUG] sequences={len(sizes)}, "
            f"min_turns={min(sizes)}, max_turns={max(sizes)}, "
            f"example={all_sequences_turn_data[0] if len(all_sequences_turn_data) else None}")

        # ─── 3.  tensor post‑processing ───────────────────────────────────
        output_batch = self.completion_callback.postprocess(
            batch, batch_conversations, n=n
        )

        # ─── 4.  rebuild non‑tensor part  ─────────────────────────────────
        is_val    = batch.meta_info.get("validate", False)
        max_turns = self.completion_callback.max_turns

        # ─── 4‑a) build rewards_data  ────────────────────────────────────────
        def to_summary(td) -> dict:
            "Convert any shape to the flat summary schema used downstream."
            if isinstance(td, dict) and "acc" in td:
                return td                              # already a summary
            elif isinstance(td, dict):                 # dict‑of‑turns
                turns = [td[k] for k in sorted(td)]
            elif isinstance(td, list) and td and isinstance(td[0], dict):
                turns = td                             # list‑of‑turn dicts
            else:                                      # bare float / empty
                turns = [{"score": float(td), "acc": float("nan")}]

            last   = turns[-1]
            accs   = [t.get("acc", float("nan")) for t in turns]
            while len(accs) < max_turns:
                accs.append(accs[-1])

            summary = {
                "turn":  len(turns),
                "score": last.get("score", float("nan")),
                "acc":   accs[-1],
                "max_turns": last.get("max_turns", len(turns)),
                "mean_verification_time": float(
                    np.mean([t.get("verification_time", 0.0) for t in turns])
                ),
                "terminate_reason": last.get("terminate_reason", "unknown"),
                "pred": last.get("pred"),
            }
            for t in range(1, max_turns + 1):
                summary[f"acc_turn_{t}"] = accs[t - 1]
            return summary

        rewards_data = [to_summary(td) for td in all_sequences_turn_data]
        # ─────────────────────────────────────────────────────────────────────


        # 4‑b) duplicate other metadata when n > 1 (train batches)
        non_tensor_out = {}
        for k, v in batch.non_tensor_batch.items():
            if k == "raw_prompt":                 # already handled below
                continue
            if is_val:
                non_tensor_out[k] = v
            else:
                if isinstance(v, np.ndarray):
                    non_tensor_out[k] = np.repeat(v, n, axis=0)
                else:
                    non_tensor_out[k] = v * n     # simple list / scalar duplication

        non_tensor_out["raw_prompt"] = batch.non_tensor_batch["raw_prompt"].repeat(n, axis=0)
        non_tensor_out["turn_info"]  = np.array(rewards_data, dtype=object)

        output_batch.non_tensor_batch = non_tensor_out
    ...
    async def generate_sequences_v2(self, batch: DataProto) -> DataProto:
        t_start = time.time()
        kwargs = dict(
            model=self.model_name,
            temperature=self.config.temperature,
        ...
        # asserts
        bsz = output_batch.batch["input_ids"].shape[0]

        for name, field in output_batch.non_tensor_batch.items():
            assert len(field) == bsz, (
                f"[metadata mis‑alignment] field '{name}' has length {len(field)}, "
                f"but tensors have {bsz}.  Did you forget to duplicate when n>1?"
            )

        # optional: schema check for turn_info
        for i, info in enumerate(output_batch.non_tensor_batch["turn_info"]):
            assert isinstance(info, dict) and {"score", "acc"}.issubset(info), (
                f"[turn_info malformed] entry {i}: {info}"
            )
